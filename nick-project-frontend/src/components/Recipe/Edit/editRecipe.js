    ...
    useEffect(() => {
        if (confirmLeaving) {
            ...
            RecipeRepository.getRecipe(recipeId).then(promise => {
                ...
                document.title = `${recipe.name} - Промени`;
            });
        }
            ...
            <div className="form-group">
                ...
                <div className="row">
                    ...
                    <div className="col-12 col-sm-6 mt-2 mt-sm-0 text-center">
                        <span className="lead">
                            {recipe.imageFiles.length + images.length} слики
                        </span>
        ...
        const reminderModal = () => {
            ...
            return (
                <Modal show={showReminderModal} onHide={handleCloseReminderModal}
                    ...
                    <Modal.Body>
                        Доколку излезете од страната без да притиснете на копчето "ПРОМЕНИ", тоа што сте го промениле ќе
                        се изгуби. Дали сте сигурни дека сакате да излезете?
                    </Modal.Body>
                    <Modal.Footer>
